...
public class RotationControl extends AbstractControl {
    ...
    public RotationControl(Controller controller) {
        ...
        try {
            cursor = Toolkit.getDefaultToolkit().createCustomCursor(SvgLoader.loadIcon("img/cursors/rotate.svg", 16).map(ImageIcon::getImage).orElse(null), new Point(8, 8), "rotater");
    ...
    public void render(Graphics2D graphics, Drawing drawing) {
        ...
        if (getSelectionManager().getSelection().size() == 1 &&
                getSelectionManager().getSelection().get(0) instanceof com.willwinder.ugs.designer.entities.cuttable.Point) {
            return;
        }
