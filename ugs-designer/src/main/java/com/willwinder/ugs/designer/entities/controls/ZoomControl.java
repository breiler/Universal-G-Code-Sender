package com.willwinder.ugs.designer.entities.controls;
...
public class ZoomControl extends AbstractControl {
    ...
    public ZoomControl(Controller controller) {
        ...
        zoomInCursor = Toolkit.getDefaultToolkit().createCustomCursor(SvgLoader.loadImageIcon("img/zoom-in.svg", 16).map(ImageIcon::getImage).orElse(null), new Point(8, 8), "zoom-in");
        zoomOutCursor = Toolkit.getDefaultToolkit().createCustomCursor(SvgLoader.loadImageIcon("img/zoom-out.svg", 16).map(ImageIcon::getImage).orElse(null), new Point(8, 8), "zoom-in");
